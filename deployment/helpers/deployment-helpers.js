...
async function deployCDKValidiumDeployer(deployerAddress, signer) {
    ...
    const gasPrice = hre.network.name === 'hardhat'
        ? ethers.BigNumber.from(ethers.utils.parseUnits("5", 'gwei'))
        : maxFeePerGas;
...
console.log("step 1")
    await (await signer.sendTransaction(params)).wait(2);
console.log("step 2")
...
async function create2Deployment(cdkValidiumDeployerContract, salt, deployTransaction, dataCall, deployer, hardcodedGasLimit) {
    ...
    if (dataCall) {
        ...
        if (hardcodedGasLimit) {
            ...
            populatedTransaction.gasPrice = ethers.utils.parseUnits("5", "gwei");
            await (await deployer.sendTransaction(populatedTransaction)).wait();
        } else {
            await (await cdkValidiumDeployerContract.deployDeterministicAndCall(amount, salt, deployTransaction, dataCall,
		{
            	type: 0,
            	gasPrice: ethers.utils.parseUnits("5", "gwei"),
          	})).wait();
        ...
        if (hardcodedGasLimit) {
            ...
            await (await cdkValidiumDeployerContract.deployDeterministic(amount, salt, deployTransaction,
		{
            	type: 0,
            	gasPrice: ethers.utils.parseUnits("5", "gwei"),
          	})).wait();
