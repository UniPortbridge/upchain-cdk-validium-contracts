...
async function main() {
    ...
    currentProvider = new ethers.providers.JsonRpcProvider(`https://data-seed-prebsc-1-s1.binance.org:8545`);
    async function overrideFeeData() {
        return {
	...
	console.log("Load cdkValidium deployer");
    ...
    if (!ongoingDeployment.verifierContract) {
      ...
      gasPrice: ethers.utils.parseUnits("5", "gwei"),
		...
		console.log("FflonkVerifier end...")
        } else {
		console.log("VerifierRollupHelperMock...")
      ...
      gasPrice: ethers.utils.parseUnits("5", "gwei"),
            ...
            await verifierContract.deployed();
		console.log("VerifierRollupHelperMock end...")
        }
        console.log('#######################\n');
    ...
    const overrideGasLimit = ethers.BigNumber.from("8500000"); //new bridge need more gas, old is 5500000
    ...
    //zk
    const gasTokenMetadata = await ethers.utils.defaultAbiCoder.encode(['string', 'string', 'uint8'], ['Ether', 'ETH', 18]);
    
    //G
    //const gasTokenMetadata = await ethers.utils.defaultAbiCoder.encode(['string', 'string', 'uint8'], ['USD Coin', 'USDC', 6]);
    //const gasTokenDecimalDiffFactor =  ethers.BigNumber.from(10).pow(process.env.GASTOKEN_DECIMAL_DIFF); //GASTOKEN_DECIMAL_DIFF=12
    ...
    for (let i = 0; i < attemptsDeployProxy; i++) {
        try {
		    ...
		    {type:0,gasPrice: ethers.utils.parseUnits("5", "gwei")},
            );
            break;
        } catch (error) {
    ...
    const outputJson = {
        ...
        polygonZkEVMBridgeAddressL2 : process.env.BRDGE_ADDR_L2,
    ...
    var contract_params = fs.readFileSync("./deploy/contract_params.sh");
    contract_params = contract_params + "\nexport GLOBAL_PARAM_cdkValidiumAddress="+cdkValidiumContract.address;
    contract_params = contract_params + "\nexport GLOBAL_PARAM_polygonZkEVMBridgeAddress="+PolygonZkEVMBridgeContract.address;
    contract_params = contract_params + "\nexport GLOBAL_PARAM_L2_polygonZkEVMBridgeAddress="+process.env.BRDGE_ADDR_L2;
    contract_params = contract_params + "\nexport GLOBAL_PARAM_polygonZkEVMGlobalExitRootAddress="+PolygonZkEVMGlobalExitRoot.address;
    contract_params = contract_params + "\nexport GLOBAL_PARAM_cdkDataCommitteeContract="+cdkDataCommitteeContract.address;
    contract_params = contract_params + "\nexport GLOBAL_PARAM_genBlockNumber="+deploymentBlockNumber;
    contract_params = "\n" + contract_params;
    fs.writeFileSync("./deploy/contract_params.sh",contract_params);
