...
async function main() {
    ...
    console.log("begin to deploy Matic")
    ...
    var contract_params = "#!/bin/bash\n";
    contract_params = contract_params + "export GLOBAL_PARAM_maticTokenAddress="+maticTokenContract.address;
	...
	console.log("begin transfer bnb:"+balanceEther)
    ...
    if (balanceEther < minEtherBalance) {
        ...
        await (await deployer.sendTransaction(params)).wait(2);
    }
	console.log("begin transfer matic")
    const tokensBalance = ethers.utils.parseEther('100000');
    await (await maticTokenContract.transfer(trustedSequencer, tokensBalance,{ type: 0,
      gasPrice: ethers.utils.parseUnits("5", "gwei")})).wait(2);
