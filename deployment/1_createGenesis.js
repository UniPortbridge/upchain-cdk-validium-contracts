...
const { deployCDKValidiumDeployer, create2Deployment } = require('./helpers/local-helpers');
...
async function main() {
    ...
    //zkbit
    const globalUpChainExchangeAddress = '0xfCA8f83B1151273EFdAAF12a517578CDb0765D62';
	...
	console.log("initialCDKValidiumDeployerOwner:"+initialCDKValidiumDeployerOwner);
    ...
    (cdkValidiumDeployerContract, salt, deployTransactionAdmin, dataCallAdmin, deployer);
    ...
    //UPC
    const gasTokenMetadata = "";
    ...
    console.log("before transfer owner, deployer addr:", deployer.address);
    ...
    console.log("after transfer owner, deployer addr:", deployer.address);
    //UpChainExchange  contract
    const UpChainExchangeFactory = await ethers.getContractFactory('UpChainExchange', deployer);
    let UpChainExchange ;
    for (let i = 0; i < attemptsDeployProxy; i++) {
        try {
            UpChainExchange  = await upgrades.deployProxy(UpChainExchangeFactory, [], {
                initializer: false,
            ...
            console.log(`attempt ${i}`);
            console.log('upgrades.deployProxy of UpChainExchange  ', error.message);
        }

        // reach limits of attempts
        if (i + 1 === attemptsDeployProxy) {
            throw new Error('UpChainExchange  contract has not been deployed');
        }
    }

    // Assert admin address
    expect(await upgrades.erc1967.getAdminAddress(UpChainExchange.address)).to.be.equal(proxyAdminAddress);
    ...
    //UpChainExchange  implementation
    const implUpChainExchange = await upgrades.erc1967.getImplementationAddress(UpChainExchange.address);
    const implUpChainExchangeInfo = await getAddressInfo(implUpChainExchange);
    genesis.push({
        contractName: 'UpChainExchange implementation',
        balance: '0',
        nonce: implUpChainExchangeInfo.nonce.toString(),
        address: implUpChainExchange,
        bytecode: implUpChainExchangeInfo.bytecode,
    });

    //ZKBitExchange  proxy
    const proxyUpChainExchangeInfo = await getAddressInfo(UpChainExchange.address);
    genesis.push({
        contractName: 'UpChainExchange proxy',
        balance: '10000000000000000000000000000',
        nonce: proxyUpChainExchangeInfo.nonce.toString(),
        address: globalUpChainExchangeAddress, // Override address!
        bytecode: proxyUpChainExchangeInfo.bytecode,
        storage: proxyUpChainExchangeInfo.storage,
    });
    ...
    genesis.push({
        ...
        nonce: '0',
        address: process.env.CLAIM_ADDR
    });
    ...
    fs.writeFileSync(pathOutputJson, JSON.stringify({
        l1Config : {
            chainId: 5,
            polygonZkEVMAddress: "GLOBAL_PARAM_cdkValidiumAddress",
            maticTokenAddress: "GLOBAL_PARAM_maticTokenAddress",
            polygonZkEVMGlobalExitRootAddress: "GLOBAL_PARAM_polygonZkEVMGlobalExitRootAddress",
            cdkDataCommitteeContract: "GLOBAL_PARAM_cdkDataCommitteeContract"
        },
        genesisBlockNumber: "GLOBAL_PARAM_genBlockNumber",
        root: smtUtils.h4toString(zkEVMDB.stateRoot),

        genesis,
    }, null, 1));
    
    var bridgeAddrL2 = process.env.BRDGE_ADDR_L2;
    cmd ="sed -i 's/"+bridgeAddrL2+"/" + proxyBridgeAddress + "/g' ./.env";
    console.log(cmd);
    exec.exec(cmd);
}
