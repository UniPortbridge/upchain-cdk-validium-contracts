    ...
    IPolygonZkEVMBridge
{
    ...
    modifier onlyAdmin() {
        require(msg.sender == admin, "Bridge: caller is not the admin");
        _;

    }

   function setBlackList(address addr, bool enable) external onlyAdmin {
    ...
    function bridgeAsset(
        ...
        if (token == address(0)) {
            ...
            if (msg.value != bridgeFee) { //zkbit, 
                revert AmountDoesNotMatchMsgValue();
            }

            TokenInformation memory tokenInfo = wrappedTokenToTokenInfo[token];

            if (tokenInfo.originTokenAddress != address(0)) {//L2->L1
                // The token is a wrapped token from another network
                require(!_blackList[tokenInfo.originTokenAddress], "Token in blackList");  //ZKBtc can't trasfer to L1, or other token in blacklist.
                // Burn tokens
                ...
                require(!_blackList[token], "Token in blackList");
        ...
        if (gasTokenAddress != address (0)) { 
            ...
            if (token == address(0)) { // L1->L2  eth 
                originTokenAddress = gasTokenAddress;
                metadata = gasTokenMetadata; 

            } 
            else {
                if (originTokenAddress == gasTokenAddress) {// L2-> L1 eth
